  ...
  try {
    ...
    if (pathname === '/api/auth/claim-guest-roadmaps' && method === 'POST') {
      ...
      if (Array.isArray(roadmapIds) && roadmapIds.length > 0) {
        ...
        writeJSON(ROADMAPS_FILE, roadmaps);
        return sendJSON(res, 200, { success: true, claimed: updatedCount });
      }
    ...
    if (pathname === '/api/roadmaps' && method === 'GET') {
      ...
      const userRoadmaps = roadmaps.filter(r => r.userId === effectiveUserId || (!authUser && r.userId === 'usr_guest'));
    ...
    if (matchRoadmapId && method === 'PUT') {
      ...
      if (index === -1) return sendJSON(res, 404, { error: 'Roadmap not found' });

      // If user is authenticated, ensure ownership
      if (authUser && roadmaps[index].userId && roadmaps[index].userId !== authUser.id && roadmaps[index].userId !== 'usr_guest') {
        return sendJSON(res, 403, { error: 'Forbidden: You do not own this roadmap' });
      }

      if (body.title) roadmaps[index].title = body.title;
      if (body.goal) roadmaps[index].goal = body.goal;
      if (body.targetRole !== undefined) roadmaps[index].targetRole = body.targetRole;
      if (body.nodes && Array.isArray(body.nodes)) {
        roadmaps[index].nodes = body.nodes.map(validateNode);
      }
      roadmaps[index].updatedAt = new Date().toISOString();

      writeJSON(ROADMAPS_FILE, roadmaps);
      return sendJSON(res, 200, roadmaps[index]);
    }
    ...
    if (matchRoadmapId && method === 'DELETE') {
      ...
      writeJSON(ROADMAPS_FILE, filtered);
